...
// Defined at module scope (NOT inside LivePitchRoom) so it keeps a stable
// component identity across renders. When this lived inside the render body,
// every state update during a pitch (the 1s timer, mic activity, each chat /
// transcript / audio message, speaker changes) gave it a new function identity,
// so React unmounted and remounted the <video> on every render — detaching and
// reattaching the camera MediaStream and making the feed skip/stutter. memo
// keeps it from re-rendering unless its actual inputs change.
...
export default function LivePitchRoom() {
  ...
  // The WebSocket message handler captures props/state in a closure that does
  // NOT re-bind on every verdictPhase change, so reading the state directly
  // there is stale. This ref is always current and is what the handler checks.
  const verdictPhaseRef = useRef(false);
  useEffect(() => {
    verdictPhaseRef.current = verdictPhase;
  }, [verdictPhase]);
    ...
    const startAudioCapture = async () => {
      try {
        ...
        processor.onaudioprocess = (e) => {
          ...
          // Convert to base64. Chunked apply() is dramatically cheaper than
          // building the binary string one char at a time — that per-byte loop
          // ran every ~256ms on the main thread and starved audio playback,
          // contributing to the crackle.
    ...
    const handleMessage = async (event: MessageEvent) => {
      try {
        ...
        if (data.type === "stop_audio") {
          if (verdictPhaseRef.current) return;
          activeSourcesRef.current.forEach((src) => {
            try {
              src.stop();
            } catch (e) {}
        ...
        if (data.type === "audio") {
          ...
          if (hasAudio) {
            ...
            // Route through a per-chunk gain envelope. Each sentence is a
            // separate TTS synthesis, so back-to-back buffers don't line up at
            // zero crossings — without a short fade you hear a click/pop at
            // every seam (the "cracking"). Fading in/out kills the seam.
            ...
            // Small safety lead so we never schedule in the past, but small
            // enough that a slightly-late chunk doesn't insert audible silence.
            if (startTime < currentTime + 0.02) {
              startTime = currentTime + 0.02;
            }

            const dur = audioBuffer.duration;
            const fade = Math.min(0.012, dur / 2); // ~12ms fade in/out
            gainNode.gain.setValueAtTime(0.0001, startTime);
            gainNode.gain.linearRampToValueAtTime(1, startTime + fade);
            gainNode.gain.setValueAtTime(
              1,
              Math.max(startTime + fade, startTime + dur - fade),
            );
            gainNode.gain.linearRampToValueAtTime(0.0001, startTime + dur);

            activeSourcesRef.current.push(source);
            source.start(startTime);
            nextStartTimeRef.current = startTime + dur;
            ...
            if (hasText) {
              ...
              const textTimeout = window.setTimeout(() => {
                setActiveSpeakerName(speaker);
                setMessages((prev) => {
                  // Only merge into the previous bubble if we're still inside
                  // the same turn. A new turn (incl. an idle nudge from the same
                  // panelist) must start a fresh bubble instead of editing the
                  // last one.
                  if (prev.length > 0 && !turnStartedRef.current) {
                    const last = prev[prev.length - 1];
                    if (last.type === "ai" && last.speaker === speaker) {
    ...
    if (socket && socket.readyState === WebSocket.OPEN) {
      socket.send(
        JSON.stringify({
          ...
          // Backend reads `panelists`; sending under the right key so the
          // configured personas (e.g. Riley in coach mode) give the verdicts
          // instead of the default Marcus/Sarah/Chen panel.
          panelists: pitchConfig
  ...
  // CameraViewer is defined at module scope (above) to keep the <video> element
  // mounted across re-renders; do NOT redefine it here.
