...
export function initRestSocket(wss: WebSocketServer) {
  wss.on("connection", async (ws) => {
    ...
    let verdictStarted = false;

    // Gate so the first *real* user turn never runs before the full
    // (persona + deck) master prompt has been built. The greeting does NOT
    // wait on this — it uses a lightweight synchronous prompt instead.
    let resolveMasterPromptReady: () => void = () => {};
    const masterPromptReady = new Promise<void>((res) => {
      resolveMasterPromptReady = res;
    });
    ...
    const processAiTurn = async (turn: QueuedTurn) => {
      ...
      try {
        ...
        // The greeting uses a lightweight synchronous prompt (built below) and
        // must NOT wait on deck resolution. Every other turn must wait so it
        // always has full persona + deck context.
        if (!turn.isGreeting) {
          await masterPromptReady;
        }
      ...
      try {
        ...
        if (data.type === "client_ready" && !hasSentSetup) {
          ...
          resolveDeckText(clientConfig)
            ...
            .catch((err) => {
              console.error("Error resolving deck text:", err);
              resolvedDeckText = "";
            })
            .finally(() => {
              // Build the full master prompt with whatever deck text we have
              // (empty string is handled gracefully downstream) and open the gate.
          ...
          if (hasAzureSttConfig()) {
            sttRecognizer = createStreamingRecognizer((text) => {
              ...
              (partialText) => {
                if (sessionEnded || verdictStarted) return;
                // Require a couple of recognized characters before treating a
                // partial result as a real barge-in. This stops single-syllable
                // noise, breaths, and the AI's own voice echo (when the user is
                // not on headphones) from interrupting playback / the flow.
                if (!partialText || partialText.trim().length < 6) return;
                const now = Date.now();
                const last = (sttRecognizer as any)?._lastInterruption || 0;
                // Avoid spamming stop_audio too fast
                if (now - last > 2000) {
                  if (sttRecognizer) (sttRecognizer as any)._lastInterruption = now;
        ...
        if (data.type === "verdict_request") {
          ...
          const panelists = data.panelists || data.personas || [
