...
export const useScreenCapture = (onFrame?: (base64: string, slideIndex: number) => void) => {
  ...
  const captureFrame = useCallback(() => {
    // No consumer = nothing to do. The expensive drawImage + toDataURL below
    // runs on the main thread, so skipping it when unused prevents the periodic
    // hitch that made the camera preview stutter ("crack") every interval.
    if (!onFrame || !videoRef.current || !canvasRef.current) return;
  ...
  const startCapture = useCallback(async () => {
    try {
      ...
      // Only run the frame-grab loop if something actually consumes frames.
      // Screen-share *display* uses the live MediaStream directly and does not
      // need this loop, so we avoid the per-interval main-thread encode.
      if (onFrame) {
  ...
  }, [captureFrame, stopCapture, onFrame]);
